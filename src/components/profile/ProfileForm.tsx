...
export default function ProfileForm() {
  ...
  const handleSave = () => {
    ...
    setIsEditing(false);
    alert(t('save_success') || 'Профиль сохранён');
  };
      ...
      {!isEditing ? (
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-4">
          ...
          <div>
            ...
            <select name="language" value={formData?.language || 'ru'} onChange={handleChange} className="w-full px-3 py-2 border rounded-md dark:bg-gray-800 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" name="showLastSeen" checked={formData?.privacySettings.showLastSeen || false} onChange={handlePrivacyChange} />
            <label>{t('privacy_show_last_seen')}</label>
          </div>
          <div className="flex items-center gap-2">
            <input type="checkbox" name="showReadReceipts" checked={formData?.privacySettings.showReadReceipts || false} onChange={handlePrivacyChange} />
